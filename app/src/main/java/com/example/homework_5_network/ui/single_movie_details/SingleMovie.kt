...
import com.example.homework_5_network.R
import com.example.homework_5_network.data.api.POSTER_BASE_URL
import com.example.homework_5_network.data.api.TheMovieDBClient
...
class SingleMovie : AppCompatActivity() {
    ...
    fun bindUI(it: MovieDetails){
        movie_title.text = it.title
        movie_tagline.text = it.tagline
        movie_releade_data.text = it.releaseDate
        movie_rating.text = it.rating.toString()
        movie_runtime.text = it.runtime.toString() + " minutes"
        movie_overview.text = it.overview
    }
